...
describe Nexpose::IPRange do
  ...
  describe '#include?' do
    ...
    context 'with a Nexpose::IPRange argument' do
      let(:ip_range) { Nexpose::IPRange.new('192.168.1.64','192.168.1.127') }
      it 'returns a bool for IPRange of a single address'
      it 'returns a bool for IPRange spanning multiple addresses'
    end
    context 'with a castable string argument' do
      let(:ip_range) { Nexpose::IPRange.new('192.168.1.64','192.168.1.127') }
      it 'returns bool for single IPv4 addresses'
      it 'returns bool for masked IPv4 addresses'
      it 'returns bool for single IPv6 addresses'
      it 'returns bool for masked IPv6 addresses'
    end
    ...
    context 'calling from a spanning range' do
      ...
      context 'with a single IPv4 argument' do
        ...
        it 'returns true if self.from==other' do
          ...
          expect(span_10_100.include_ipaddr?(equal_from)).to be_truthy
        end
        it 'returns true if self.to==other' do
          equal_to = IPAddr.new('192.168.1.100')
          expect(span_10_100.include_ipaddr?(equal_to)).to be_truthy
        end
        it 'returns true if self.from < other < self.to' do
          inside = IPAddr.new('192.168.1.50')
          expect(span_10_100.include_ipaddr?(inside)).to be_truthy
        end
      end
